...
**Grant outlay data unavailable via USASpending API — all paths exhausted (notebook 04).** Three endpoints tested: (1) `/api/v2/awards/{id}/` — `total_outlays` field present but null for all 12,361 grants; (2) `/api/v2/awards/funding/` — HTTP 200, zero result rows; (3) `spending_by_award` search with FAINs — zero results across all assistance type groups. Working hypothesis: cancelled awards are purged from USASpending's search index but remain accessible via direct award lookup.

**Three-number record — grants (notebook 05).** Across 12,361 matched grants: ceiling $54.2B / obligated $35.7B / outlays $23.5B. Three-number completeness: 67.9% (8,389 of 12,361). 10 FAINs unmatched across all archive years — effectively complete coverage. Obligation cross-check (bulk vs. API): 11,975/12,351 within 1% — high consistency. Ceiling-to-obligation gap: median 0.0% (contrast with contracts: 42.5%) — most grants were fully obligated at cancellation. 15.0% of grants had >50% unobligated at cancellation. Outlay rate: median 64.6%, 37.4% of grants were >80% disbursed. Outlay source: `total_outlayed_amount_for_overall_award` field from USASpending `Assistance_PrimeTransactions` annual archive files (files.usaspending.gov); max() per FAIN across transactions (cumulative field). Delta file (FY(All)) and FY2026–FY2024 full files yielded 0 matches for the remaining 1,201 FAINs — these grants' last transactions are in FY2023 and earlier, confirming the delta file is incremental (not cumulative) and that cancellation events for older grants may not generate new USASpending transaction records.
...
- Grant outlay data — resolved (notebook 05): $23.5B in outlays recovered from archive files; 67.9% three-number completeness; 10 FAINs permanently unmatched
