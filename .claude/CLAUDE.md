...
- Grant outlay data — all three USASpending API paths exhausted (notebook 04): (1) `/api/v2/awards/{id}/` returns `total_outlays: null`; (2) `/api/v2/awards/funding/` returns HTTP 200 with zero rows; (3) `spending_by_award` search with FAINs returns zero results across all assistance type groups. Working hypothesis: cancelled awards are purged from USASpending's search index but remain accessible via direct award lookup, explaining why `total_obligation` is present but outlay data is not. Remaining path: USASpending bulk download (`Assistance_PrimeTransactions` CSV). Join path: FAIN (extracted as `ASST_{TYPE}_{FAIN}_{AGENCY_CODE}`) against `award_id_fain` field in bulk CSV. Deferred pending assessment of whether grant outlays are worth the download cost.
